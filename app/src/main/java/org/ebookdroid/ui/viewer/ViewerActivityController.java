            ...
            public void run() {
                try {
                	if (currentlyDecoding > 0){
                		activity.mCurrentUIHandler.sendEmptyMessage(ViewerActivity.MESSAGE_SHOW_LOADINGBAR);
                	}
                	else
                		activity.mCurrentUIHandler.sendEmptyMessage(ViewerActivity.MESSAGE_HIDE_LOADINGBAR);
...
//                            currentlyDecoding == 0 ? 10000 : currentlyDecoding);
    ...
    public void currentPageChanged(final PageIndex oldIndex, final PageIndex newIndex) {
        ...
        getManagedComponent().currentPageChanged((newIndex.viewIndex + 1), pageCount, bookTitle);
    ...
    public void onBookSettingsChanged(final BookSettings oldSettings, final BookSettings newSettings,
        ...
        if (diff.isViewModeChanged() || diff.isSplitPagesChanged() || diff.isCropPagesChanged()) {
            redrawn = true;
            newSettings.viewMode=DocumentViewMode.SINGLE_PAGE;
